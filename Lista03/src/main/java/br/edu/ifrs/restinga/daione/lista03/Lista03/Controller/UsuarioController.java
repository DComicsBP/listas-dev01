    ...
    public Iterable<Usuario> getUsuarios() {
        ...
        if (users != null) {
            for (Usuario usuario : users) {
                usuario.setTelefone(tDAO.findByUser(usuario));
                usuario.setEmprestimos(eDAO.findByUsuario(usuario));
            }
        } else {
            throw new ERROR400("Não foi possível carregar a lista de usuarios.");
        }
